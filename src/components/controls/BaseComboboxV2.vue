...
/**
 * BaseComboboxV2.vue
 * Gộp toàn bộ state/logic của useComboboxStore.ts thẳng vào component – không còn Pinia store riêng.
 * ComboboxStoreOptions cũ -> props trực tiếp. comboboxLoadData cũ -> emit('load-data', payload, done),
 * parent lắng nghe event, tự gọi API rồi done(items) để component nhận lại data.
 * Các hàm reset/loadData/loadNextPage/setSelectedItem/clearValue được return từ setup() nên
 * parent vẫn gọi được qua template ref.
 */
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const loadData = async (keyword: string, options: any = null): Promise<void> => {
      ...
      if (!isRemoteMode.value) {
        ...
        if (!keyword || !df) {
          ...
          data.value = rawData.value.filter((item) =>
            String(item[df]).toLowerCase().includes(kw)
          );
        }
        return;
      }
      ...
      loading.value = true;
      try {
        const result = await requestRemoteData(buildPayload(1));
    ...
    const loadNextPage = async (): Promise<void> => {
      ...
      try {
        const result = await requestRemoteData(buildPayload(nextPage));
      ...
      if (valueOrItem != null) {
        item =
          typeof valueOrItem === "object"
            ? valueOrItem
            : storeData.value.find(
                (i) => i[props.valueField] === valueOrItem || i === valueOrItem
              ) ?? null;
      }
